...
class Loader {
  ...
  async _requireOrImport(file) {
    if (process.platform === 'win32') file = await fixWin32FilepathCapitalization(file);
...
async function fixWin32FilepathCapitalization(file) {
  /**
   ...
   */
  const realFile = await new Promise((resolve, reject) => fs.realpath.native(file, (error, realFile) => {
  ...
  if (realFile.toLowerCase() === file.toLowerCase()) return realFile;
  return file;
}